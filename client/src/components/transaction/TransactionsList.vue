<template>
	<v-row v-if="!loading">
		<v-col cols="12">
			...
			<transaction-list-item v-for="item in transactions" :key="item.id" :transaction="item" />
			</v-fade-transition>
		</v-col>
	</v-row>
	<v-row v-else>
...
@Component({
	components: {
		TransactionListItem: () => import('@/components/transaction/TransactionListItem.vue')
	}
})
