      ...
      <v-col cols="6">
        <section>
          <v-card>
            ...
            <v-card-text>
              <v-row>
                <v-col cols="12">Código</v-col>
                <v-col cols="12">
                  <v-text-field solo hide-details label="Código da conta" disabled :value="account.id" />
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12">Nome</v-col>
                <v-col cols="12">
                  <v-text-field solo hide-details label="Nome da conta" v-model="account.name" />
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="12">Tipo</v-col>
                <v-col cols="12">
              ...
              <v-row v-if="account.institution && account.institution.name == 'Outro'">
                <v-col cols="12">Outra Instituição</v-col>
                <v-col cols="12">
                  <v-text-field solo hide-details label="Banco ou corretora" v-model="account.unregisteredInstitution" />
                </v-col>
              </v-row>
            </v-card-text>
          </v-card>
...
export default class EditDebitAccount extends Vue {
  ...
  get institution(): Institution | null {
    ...
    if (typeof inst == "string") {
      const institution = finances.institutions.find((i) => i.id == inst);
